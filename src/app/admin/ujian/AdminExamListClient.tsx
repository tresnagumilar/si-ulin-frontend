              ...
              <div key={exam.id} className="bg-white rounded-2xl p-4 border border-gray-200 shadow-sm space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-0.5">Guru: <span className="font-semibold text-gray-700">{exam.teacher?.name || '-'}</span></p>
                    <p className="text-xs text-gray-500">{exam.durationMin} Menit • {exam.questions_count || 0} Soal • {exam.attempts_count || 0} Siswa</p>
                  </div>
                  <button 
                ...
                <div className="pt-2 border-t border-gray-100 flex items-center justify-between gap-1 overflow-x-auto">
                  <div className="flex items-center gap-1.5">
                    {exam.isLive && (
                      <Link href={`/admin/ujian/${exam.id}/live`} className="px-2.5 py-1.5 text-xs font-bold text-white bg-red-500 rounded-lg flex items-center gap-1 shadow-sm shrink-0">
                        <Activity className="w-3.5 h-3.5" /> Monitor
                      </Link>
                    )}
                    <Link href={`/admin/ujian/${exam.id}/hasil`} className="px-2.5 py-1.5 text-xs font-bold text-orange-700 bg-orange-50 hover:bg-orange-100 rounded-lg flex items-center gap-1 border border-orange-100 shrink-0">
                      <Eye className="w-3.5 h-3.5" /> Rekap
                    </Link>
                    <Link href={`/admin/ujian/${exam.id}`} className="px-2.5 py-1.5 text-xs font-bold text-primary-blue bg-blue-50 hover:bg-blue-100 rounded-lg flex items-center gap-1 border border-blue-100 shrink-0">
