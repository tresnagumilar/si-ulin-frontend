...
export default async function LiveMonitorPage({ params }: { params: Promise<{ id: string }> }) {
  const session = (await getServerSession(authOptions)) as any;
  
  if (!session || !session.user || ((session.user as any).role !== 'GURU' && (session.user as any).role !== 'ADMIN')) {
    redirect('/');
  }
  ...
  const backLink = (session.user as any).role === 'ADMIN' ? '/admin/ujian' : '/guru/ujian';

  if (!res.ok) {
    return (
      <div className="p-8 max-w-7xl mx-auto text-center">
        <h2 className="text-2xl font-bold text-gray-800">Ujian tidak ditemukan</h2>
        <Link href={backLink} className="text-primary-blue hover:underline mt-4 inline-block">Kembali ke Daftar</Link>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto">
      <Link href={backLink} className="inline-flex items-center gap-2 text-gray-500 hover:text-primary-blue transition-colors font-medium mb-6">
      ...
      <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-gray-500 font-medium">{exam.subject} • Pengawasan Real-Time</p>
          </div>
        </div>
      </div>
