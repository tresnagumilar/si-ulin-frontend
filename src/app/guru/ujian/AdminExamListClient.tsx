...
import { Plus, Settings, Eye, Trash2, Edit, DownloadCloud, Activity, RotateCw } from 'lucide-react';
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showAlert('Ujian baru berhasil dibuat!', 'Berhasil', 'success');
      } else {
        showAlert('Gagal membuat ujian baru.', 'Gagal', 'error');
      }
    } catch (error) {
  ...
  const toggleLive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      if (res.ok) {
        ...
        showAlert('Gagal mengubah status ujian.', 'Gagal', 'error');
      }
    } catch (error) {
      showAlert('Terjadi kesalahan jaringan.', 'Error', 'error');
    }
  ...
  const handleGenerateToken = async (id: string) => {
    try {
      ...
      if (res.ok) {
        ...
        showAlert('Gagal generate token.', 'Gagal', 'error');
      }
    } catch (error) {
      showAlert('Terjadi kesalahan jaringan.', 'Error', 'error');
    }
